...
if (isset($_POST['search'])) {
    if (!empty($_POST['search'])) {
        $sp=mysqli_real_escape_string($con, $_POST['search']);
        ...
        while ($row=mysqli_fetch_assoc($s_p)) {
            ...
            $nq=mysqli_real_escape_string($con, $row['new_quantity']);
            ...
            echo "<td>
            <a href='/supermarket/panel/admin/edit_product_page.php?pid=$id&pn=$pn&pt=$pt&pd=$pd&ped=$ped&pq=$nq&pcq=$cq&ps=$ps&cp=$cp&sp=$sp' title='edit product'><i class='fa fa-edit btn btn-primary'> edit product</i></a>
            <hr>
