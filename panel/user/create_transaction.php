...
if (isset($_POST['create_transaction'])) {
    if (!empty($_POST['t_s']) && !empty($_POST['t_pro']) && !empty($_POST['t_q']) && !empty($_POST['t_a']) && !empty($_POST['t_b']) && !empty($_POST['t_pri']) && !empty($_POST['t_i'])) {
        ...
        $sql="CREATE TABLE IF NOT EXISTS `transactions` (id INT AUTO_INCREMENT PRIMARY KEY, staff VARCHAR(20),supplier VARCHAR(40), invoice INT(11), product VARCHAR(40), quantity INT(11), amount_paid INT(11), balance INT(11), price INT(11), date_paid VARCHAR(20)) ";
        ...
        if ($create_transaction_table) {
            if ($t_a <= $t_b) {
                if ($t_a > 0) {
                    ...
                    $sql1="INSERT INTO `transactions` (staff,supplier,invoice,product,quantity,amount_paid,balance,price,date_paid) VALUES ('$user','$t_s','$t_i','$t_pro','$t_q','$t_a','$new_balance','$t_pri','$t_d') ";
